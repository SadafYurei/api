...
const punches = require('../images/JSON/punches.json')
const slaps = require('../images/JSON/slaps.json')
const pats = require('../images/JSON/pats.json')
const kills = require('../images/JSON/kills.json')
const winks = require('../images/JSON/winks.json')
const cdnURL = 'https://cdn-anime-images-api.hisoka17.repl.co/images/';
...
app.use('/controllers', express.static(require('path').join(dir, 'controllers')));
...
.post('/issue', (req, res) => {
  ...
  let body = { embeds: [{description: req.body.issue, title: 'New Issue', color: "15547709", footer: {text: `Submitted By: ${req.body.name?req.body.name:'Unknown'}`}}], avatar: "${cdnURL}rias.png"}
  let webhook = 'https://discord.com/api/webhooks/861685396417150986/A2tV11whxGr1fOeKsMbk3pdRy8NcwnNl0Afx84Cw76nPV2RnHR2bZG0OAfNydXR4dd8W';
  fetch(webhook, {method: 'post', body: JSON.stringify(body), headers: {"Content-Type": "application/json"}});
...
.post('/resolveissue', (req, res) => {
  ...
  if (req.body.hasOwnProperty("issue") && req.body.hasOwnProperty("name")){
    fetch('https://Anime-API.hisoka17.repl.co/issue', {method: 'post', body: JSON.stringify({...req.body}), headers: {"Content-Type": "application/json"}})
    res.cookie('sent', "true")
    res.redirect('/')
  } else {
...
.get('/img/hentai', async (req, res) => {
 let { body } = await require('node-superfetch').get('https://cdn-anime-images-api.hisoka17.repl.co/hentai');
 let file = body.url;
 let url = `${cdnURL}${file}`
  res.json({url: url})
})
...
.get('/img/nsfw/boobs', async function(req, res){
  let num = (await require('node-superfetch').get('https://cdn-anime-images-api.hisoka17.repl.co/boobs')).body.url;
...
app.endpoints = ['hug', 'kiss', 'slap', 'punch', 'wink', 'pat', 'kill', 'cuddle', 'waifu', 'hentai', 'boobs', 'lesbian'];

app.listen(3000, err => {
    if (err) throw err;
    console.log(`<----------------->\nAPI is up and running\n<----------------->\nTotal Listeners: ${app.endpoints.length}`);
  });
}
